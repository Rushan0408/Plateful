...
public class ReviewServiceImpl implements ReviewService {
    ...
    public Review createReview(User author, String restaurantId, ReviewCreateUpdateRequest review) {
        ...
        List<Photo> photos = review.getPhotoIds().stream().map(url -> {
            return Photo.builder()
                    .url(url)
                    .uploadDate((now).format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")))
                    .build();
        }).toList();
        ...
        Review reviewToCreate = Review.builder()
                ...
                .photos(photos)
                .datePosted(now.format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")))
                .lastEdited(now.format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")))
                .writtenBy(author)
                .build();
    ...
    public Review updateReview(User author, String restaurantId, String reviewId, ReviewCreateUpdateRequest review) {
        ...
        if(LocalDateTime.now().isAfter(LocalDateTime.parse(existingReview.getDatePosted()).plusHours(48))) {
            throw new ReviewNotAllowedException("Review can no longer bew edited");
        }

        existingReview.setContent(review.getContent());
        existingReview.setRating(String.valueOf(review.getRating()));
        existingReview.setLastEdited(LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")));

        existingReview.setPhotos(review.getPhotoIds().stream()
                .map(photoId -> Photo.builder()
                        .url(photoId)
                        .uploadDate(LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")))
                        .build()).toList());
    ...
    private void updateRestaurantAverageRating(Restaurant restaurant) {
        ...
        if(reviews.isEmpty()) {
            ...
            double averageRating = reviews.stream().mapToDouble(r -> Double.parseDouble(r.getRating()))
                    .average()
                    .orElse(0.0);
